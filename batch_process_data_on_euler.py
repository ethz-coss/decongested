        ...
        for EXPLORATION_METHOD in ["random", "neighbours"]:
            for IOT_NODES in [True, False]:
                for CENTRALIZED_RATIO in [0, 0.05, 0.5, 1]:
                    os.system(f'sbatch --mem-per-cpu=32G --gpus=1 --time=04:00:00 --wrap="python pre_process_data.py {N_ITER} {NEXT_DESTINATION_METHOD} {EXPLORATION_METHOD} {SAVE_PATH} {GRID} {CENTRALIZED_RATIO} {INTERNAL_SAVE_PATH}  {"--iot_nodes" if IOT_NODES else ""}"')
