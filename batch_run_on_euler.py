    ...
    for NEXT_DESTINATION_METHOD in ["simple", "random", "work-commute"]:
        ...
        for EXPLORATION_METHOD in ["random", "neighbours"]:
            for IOT_NODES in [True, False]:
                os.system(f'sbatch --mem-per-cpu=64G --time=48:00:00 --wrap="python dqn_grid_online.py {N_ITER} {NEXT_DESTINATION_METHOD} {EXPLORATION_METHOD} {SAVE_PATH} {"--iot_nodes" if IOT_NODES else ""}"')
